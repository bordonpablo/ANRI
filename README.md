# Advanced Geophysical Methods in Natural Resource Investigation (ANRI)

<p align="center">
  <img src="Assets/imagen01.jpg" alt="ANRI - Advanced Geophysical Methods in Natural Resource Investigation" width="800"/>
</p>

**Module:** 14506

**Programme:** Master Environmental and Resource Management (PO 2021)

**Department:** Faculty 2 — Environment and Natural Sciences, BTU Cottbus-Senftenberg

**Responsible:** Dr. Pablo Bordon

**Language:** English

**Credits:** 6 ECTS

**Duration:** 1 semester

**Max participants:** 15

---

## About This Course

This course provides advanced training in applied geophysical methods for the exploration and investigation of natural resources. Students acquire theoretical foundations and practical skills in a range of near-surface and airborne geophysical techniques, with emphasis on data acquisition, processing, interpretation, and integration within environmental and resource management contexts.

---

## Repository Structure

```
ANRI/
├── SYLLABUS.md          ← Learning outcomes and course contents
├── ASSESSMENT.md        ← Grading criteria and exam modalities
│
├── Units/               ← Permanent theoretical content (all semesters)
└── Semesters/           ← Semester-specific content (schedule, assignments, announcements)
```

---

## Course Units

| # | Topic |
|---|-------|
| 01 | [Introduction to Applied Geophysics](Units/Unit_01_Introduction_to_Applied_Geophysics/README.md) |
| 02 | [Ground Penetrating Radar](Units/Unit_02_Ground_Penetrating_Radar/README.md) |
| 03 | [GIS Fundamentals](Units/Unit_03_GIS_Fundamentals/README.md) |
| 04 | [Magnetometry](Units/Unit_04_Magnetometry/README.md) |
| 05 | [Electrical Resistivity Tomography](Units/Unit_05_Electrical_Resistivity_Tomography/README.md) |
| 06 | [Airborne Magnetics and Radiometrics](Units/Unit_06_Airborne_Magnetics_and_Radiometrics/README.md) |
| 07 | [Additional Topic](Units/Unit_07_Additional_Topic/README.md) |

---

## Semesters

| Semester | Status |
|----------|--------|
| [SS 2025](Semesters/2025_SS/README.md) | Completed |
| [WS 2025–2026](Semesters/2025-2026_WS/README.md) | Completed |
| [SS 2026](Semesters/2026_SS/README.md) | **Active** |
